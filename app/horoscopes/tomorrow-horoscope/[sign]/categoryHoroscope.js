...
import {defaultTimezone,defaultLanguage,encodedTimezone,getTomorrowLivetime,getUserTimeZone,getCurrencyInfo,getLatitudeInfo,getLongitudeInfo} from "@/Component/English/Horoscopes/DateFunctions/FrontEndDateFunction"



export default function CategoryComponent({ data, error,result }){
   const[tomorrowHoroscopes,setTomorrowHoroscopes]=useState([])
   const [loading, setLoading] = useState(true); 

   const livetime = getTomorrowLivetime(defaultTimezone);
...
const currentLongitudeValue=Longitude.longitude
  ...
  const fetchData = async () => {
    try {
      ...
      const url = `${process.env.NEXT_PUBLIC_SINGLE_TOMORROW_HOROSCOPE}${result}/${currentLatitudeValue}/${currentLongitudeValue}/${encodedTimezone}/${livetime}/${defaultLanguage}/${currencyValue}`;
      const response = await fetch(url);
      
      if (response.ok) {
        const result = await response.json();
        const {tomorrow_horoscopes } = result;
        setTomorrowHoroscopes(tomorrow_horoscopes)
  ...
  if (error) {
    ...
    const date = new Date(tomorrowHoroscopes?.EndDate);
    ...
    <div className="dailyhorosope_sec">
      <div className="left-panel">
            <h1 className="horoscope-tittle">
				<img src={`${process.env.NEXT_PUBLIC_HOROSCOPE_IMAGES}${signvalue}.webp`} alt="" className="left-sign-img"/> {capitalizedSign} Moon Sign Tomorrow Horoscope<span className="blue-light">
                <br/> 
        ...
        <nav className="nav-bar-main">
             ...
             <a href={`${HoroscopesUrl.CategoryBase.YesterdayHoroscope}${signvalue}`} className="mob-hide">
               ...
               <img src={`${process.env.NEXT_PUBLIC_HOROSCOPE_IMAGES}today.png`} width="16px" height="16px" alt="today"/>
                 Today</a>
             <a href={`${HoroscopesUrl.CategoryBase.TomorrowHoroscope}${signvalue}`} className="active">
...
<input type="hidden" name="score" value="2" readOnly="readOnly"/></span></h3>
     ...
     <p dangerouslySetInnerHTML={{ __html: tomorrowHoroscopes?.prediction_general }}/>
...
<input type="hidden" name="score" value="3" readOnly="readOnly"/></span></h3>
     ...
     <p dangerouslySetInnerHTML={{ __html: tomorrowHoroscopes?.prediction_career }}/>
...
<input type="hidden" name="score" value="3" readOnly="readOnly"/></span></h3>
     ...
     <p dangerouslySetInnerHTML={{ __html: tomorrowHoroscopes?.prediction_relationship }}/>
...
<input type="hidden" name="score" value="3" readOnly="readOnly"/></span></h3>
     ...
     <p dangerouslySetInnerHTML={{ __html: tomorrowHoroscopes?.prediction_money }}/>
...
<input type="hidden" name="score" value="2" readOnly="readOnly"/></span></h3>
   ...
   <p dangerouslySetInnerHTML={{ __html: tomorrowHoroscopes?.prediction_health }}/>
